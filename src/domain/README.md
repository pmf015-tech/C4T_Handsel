...
| Module | What it owns | Spec section |
|---|---|---|
| `deal/stateMachine.ts` (to be built) | Every legal deal transition; rejects the rest | Core Domain Model |
| `money/revShare.ts` | Pure rev-share computation, banker's rounding, minor units | E3 |
| `events/` (to be built) | Append-only event log write model (no UPDATE/DELETE) | Core Domain Model |
