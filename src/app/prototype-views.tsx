...
function Intro({ eyebrow, title, body, action }: { eyebrow: string; title: string; body: string; action?: ReactNode }) {
  return <div className="page-intro"><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{body}</p>{action}</div>;
}
...
export function TermSheet({ language, navigate }: Core) { const zh = language === "zh-Hant"; return <><div className="guest-bar">{zh ? "你正以訪客身份查看，請登入後回應" : "You’re viewing as guest — sign in to respond"}</div><section className="term-sheet-layout"><div><div className="version-tabs"><Button tone="secondary" onClick={() => undefined}>v1</Button><Button tone="secondary" onClick={() => undefined}>v2</Button><Button onClick={() => undefined}>v3 {zh ? "（目前版本）" : "(current)"}</Button></div><article className="term-sheet"><h1>{zh ? "條款表" : "TERM SHEET"}</h1><p>{zh ? "非約束性參考條款 · 2026 年 7 月 12 日" : "Non-binding Indicative Terms · 12 Jul 2026"}</p><TermSheetBody language={language} /></article><div className="button-row centered-row"><Button tone="secondary" onClick={() => navigate("builder")}>{zh ? "要求修改" : "Request changes"}</Button><Button onClick={() => navigate("contract")}>{zh ? "接受條款" : "Accept terms"}</Button></div></div><aside className="comments"><h2>{zh ? "留言" : "Comments"}</h2><Comment title={zh ? "收入分成" : "Revenue split"} body={zh ? "聯盟銷售嘅創作者分成可唔可以由 20% 提高到 22%？" : "Can we increase affiliate sales creator share from 20% to 22%?"} time={zh ? "Kaia · 2 小時前" : "Kaia · 2 hours ago"} /><Comment title={zh ? "里程碑時間表" : "Milestone schedule"} body={zh ? "可以將第二個銷售目標嘅到期日改到 8 月 15 日嗎？" : "Can we move Sales Target 2 due date to August 15?"} time={zh ? "Kaia · 2 小時前" : "Kaia · 2 hours ago"} /><textarea placeholder={zh ? "加入留言…" : "Add a comment..."} /></aside></section></>;
}
...
export function Deal({ language, navigate, approved, reportSent, disputeResolved }: Core & { approved: boolean; reportSent: boolean; disputeResolved: boolean }) {
  ...
  return <>
    ...
    <section className="deal-header"><div><h1>{dealName} <Badge>{zh ? "進行中" : "ACTIVE"}</Badge></h1><p>Brightside, Luma Skin, Handsel</p></div><div><b>{zh ? "下一步：" : "Next: "}{approved ? (zh ? "銷售報告到期" : "Sales report due") : (zh ? "品牌批准里程碑 2" : "Brand approves Milestone 2")}</b><Clock urgent>{approved ? (zh ? "7 日寬限期" : "7-day grace") : (zh ? "4 日後自動批准" : "Auto-approve in 4 days")}</Clock></div></section>
    ...
    {activeTab === "agent" ? <AgentConsole language={language} /> : <section className="deal-overview"><div className="activity-panel"><h2>{zh ? "活動時間線" : "Activity timeline"}</h2>{activity[language].map(([date, title, body]) => <article className="activity" key={title}><i>✓</i><div><small>{date}</small><h3>{title}</h3><p>{body}</p></div></article>)}</div><div className="metric-grid"><Metric label={zh ? "已支付總額" : "Total paid out"} value="NT$120,000" note={zh ? "合約總值嘅 50%" : "50% of total contract value"} /><Metric label={zh ? "下一個里程碑" : "Next milestone"} value={approved ? "NT$0" : "NT$60,000"} note={approved ? (zh ? "已批准" : "Approved") : (zh ? "款項已保留" : "Funds held")} /><Metric label={zh ? "收入分成" : "Revenue share"} value="18%" note={zh ? "淨銷售額" : "of net sales"} /><Metric label={zh ? "準時報告" : "Reports on time"} value={reportSent ? "4 / 4" : "3 / 3"} note={disputeResolved ? (zh ? "爭議已解決" : "Dispute resolved") : (zh ? "100% 準時" : "100% on time")} /></div></section>}
  </>;
}
...
export function Sales({ language, navigate, submitted, onSubmit }: Core & { submitted: boolean; onSubmit: () => void }) {
  ...
  return <section className="sales-layout">
    <article className="sales-form">
      <h1>{zh ? "2026 年 7 月報告" : "July 2026 report"} <Badge>{zh ? "品牌檢視" : "Brand view"}</Badge></h1>
      ...
      <div className="share-card"><span>{zh ? "創作者分成（18%）" : "Creator share (18%)"}</span><b>NT$27,540</b><small>{zh ? "提交時計算，並從已儲存嘅卡扣款。" : "Calculated on submission, charged to your saved card."}</small></div>
    ...
    <article className="history"><h2>{zh ? "報告紀錄" : "Report history"}</h2>{rows.map(([month, revenue, share, status]) => <button className="history-row" key={month} onClick={() => status === "late" && navigate("dispute")}><b>{month}</b><span>{revenue}</span><span>{share}</span><Badge tone={status === "late" ? "amber" : "green"}>{status === "late" ? (zh ? "逾期" : "Late") : status === "submitted" ? (zh ? "已提交" : "Submitted") : (zh ? "準時" : "On time")}</Badge></button>)}<button className="alert-row" onClick={() => navigate("dispute")}>{zh ? "連續兩次遲交報告會自動開啟爭議。標示差異 →" : "Two consecutive late reports open a dispute automatically. Flag discrepancy →"}</button></article>
  </section>;
}
...
export function Admin({ language, navigate, retried, onRetry }: Core & { retried: boolean; onRetry: () => void }) {
  ...
  return <div className="admin"><header className="admin-top"><Logo inverse /><b>Handsel Ops</b><input placeholder={zh ? "搜尋操作、合作或客戶…" : "Search by operation, deal, customer…"} /><Button tone="secondary" onClick={() => navigate("dashboard")}>{zh ? "返回應用程式" : "Back to app"}</Button></header><div className="admin-layout"><aside><button className="is-active">{zh ? "失敗操作" : "Failed operations"} <Badge tone="red">27</Badge></button><button>{zh ? "爭議" : "Disputes"} <Badge>8</Badge></button><button>{zh ? "審計紀錄" : "Audit log"}</button><button>{zh ? "警示" : "Alerts"} <Badge>12</Badge></button></aside><main><Intro eyebrow={zh ? "營運安全網" : "Ops safety net"} title={zh ? "失敗操作" : "Failed operations"} body={zh ? "以下操作失敗並需要處理。" : "Operations that failed and require attention."} /><section className="ops-table">{operations.map(([operation, deal, failure, age]) => <div className="ops-row" key={operation + deal}><span><b>{zh ? (operation === "Prefund charge" ? "預付款扣款" : "轉帳") : operation}</b><small>{deal}</small></span><Badge tone="red">{failure}</Badge><span>{zh ? age.replace("h", " 小時") : age}</span>{failure === "StripeChargeDeclined" ? <Button onClick={onRetry}>{zh ? "重試" : "Retry"}</Button> : <Badge tone="green">{zh ? "已記錄" : "Logged"}</Badge>}</div>)}</section></main><aside className="alerts"><h2>{zh ? "警示動態" : "Alerts feed"}</h2><p>✓ {zh ? "對帳差異已修復" : "Reconciliation mismatch repaired"}</p><p>● {zh ? "偵測到 Webhook 缺漏，已重新執行" : "Webhook gap detected — replayed"}</p><p>! {zh ? "偵測到高失敗率" : "High failure rate detected"}</p><p>✓ {zh ? "銀行帳戶已重新驗證" : "Bank account re-verified"}</p></aside></div></div>;
}
