...
| E3 milestones + reconciliation | Migration 0008 (milestone state columns + `sales_reports`), `src/lib/db/milestones.ts`, API, and deal-hub UI. **Two-party deliver→approve verified live**: creator delivered → `DELIVERED` (`MILESTONE_DELIVERED` by creator); brand saw the Approve button the creator did not and approved → `APPROVED` (`MILESTONE_APPROVED` by brand); creator calling approve → 403; unknown milestone id → 404. Pure domain complete: `money/revShare.ts` (13 tests), `milestone/milestone.ts` (16), `sales/lateness.ts` (11), `deal/revision.ts` (10) — all 100% coverage. **Sales-report submission + rev-share payout screen not built yet.** | Milestone deliver/approve verified |
