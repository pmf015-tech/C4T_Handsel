...
loop) are verified with real browser + database evidence. **E3 milestone and
sales-report slices are now wired end to end in code** (migration → party-scoped
adapter → zod API → S07 UI); an authenticated sales-report browser → Postgres
proof is still pending. The next dependency-safe slice is **E8 Vertex AI Gemini
Settlement Agent** — not Stripe.
...
| E3 milestones + reconciliation | Migration 0008, party-scoped `src/lib/db/milestones.ts` and `sales-reports.ts`, zod sales-report API, S07 tabs, live deterministic rev-share preview, report history, late timing badge, consecutive-late event, seven-day clock, rejection-with-reason transition, and append-only activity timeline. Existing two-party deliver→approve, 403 role guard, and 404 IDOR proof remain verified. Authenticated sales-report browser → Postgres proof is still outstanding. | Code-complete; real sales journey pending |
...
- New E3 UI/API work is intentionally uncommitted: activity timeline, sales-report
  adapter/API/UI, dashboard deal cards, state/clock components, S07 tabs, contract
  history/countdown/revision confirmation, term-sheet countdown/stale banner, and
  milestone rejection-with-reason. Preserve these alongside the pre-existing dirty
  auth/prototype work; do not reset or bulk-format the tree.
